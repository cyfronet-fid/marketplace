...
RSpec.describe Ess::DeployableServiceSerializer, backend: true do
  it "properly serializes a deployable service" do
    ...
    expect(serialized[:urls]).to eq(deployable_service.urls)
    ...
    expect(serialized[:publishing_date]).to eq(deployable_service.publishing_date.as_json)
    expect(serialized[:resource_type]).to eq(deployable_service.resource_type)
    expect(serialized[:public_contact_emails]).to eq(deployable_service.public_contact_emails)
    expect(serialized[:license_name]).to eq(deployable_service.license_name)
    expect(serialized[:license_url]).to eq(deployable_service.license_url)
