...
describe Jms::Subscriber do
  ...
  def mock_subscriber
    ...
    Jms::Subscriber.new("dummy_topic",
                        "dummy_login",
                        "dummy_pass",
                        "dummy_host",
                        "MPClientTest",
  ...
  def stub_good_message
    ...
    allow(Jms::ManageMessage).to receive(:new).with(json_service, eosc_registry_base, logger,
                                                    nil).and_return(manage_message_service)
  ...
  it "should receive error if connection fail" do
    ...
    expect do
      subscriber.run
    end.to raise_error(Jms::Subscriber::ConnectionError, "Connection failed!!")
  ...
  it "should receive error if queue send error frame" do
    ...
    expect do
      subscriber.run
    end.to raise_error(Jms::Subscriber::ConnectionError, "Connection error: Error")
    $stdout = original_stdout
  end
  ...
  it "should receive error if queue send error frame" do
    ...
    allow(Jms::ManageMessage).to receive(:new).with({}, eosc_registry_base, logger,
                                                    nil).and_return(manage_message_service)
