...
describe Jira::ConsoleChecker do
  ...
  describe "error_and_abort!" do
    it "should print FAIL and error type to stdout" do
      ...
      expect do
        con_checker.error_and_abort!(StandardError.new)
    ...
    it "should handle Errno::ECONNREFUSED" do
      expect(con_checker).to receive(:abort!)
      expect do
        con_checker.error_and_abort!(Errno::ECONNREFUSED.new)
      end.to output("#{' FAIL'.red}\n#{'ERROR'.red}: Could not connect to JIRA:
                      #{checker.client.jira_config['url']}\n").to_stdout
    end
    ...
    it "should handle Jira::Checker::CheckerError and return false" do
      ...
      output  = true
      expect do
        output = con_checker.error_and_abort!(Jira::Checker::CheckerError.new(message))
      end.to output("#{' FAIL'.red}\n  #{'- ERROR'.red}: #{message}\n").to_stdout
    ...
    it "should handle Jira::Checker::CheckerWarning and return false" do
      ...
      output  = true
      expect do
        output = con_checker.error_and_abort!(Jira::Checker::CheckerWarning.new(message))
    ...
    it "should handle Jira::Checker::CriticalCheckerError and abort" do
      ...
      expect(con_checker).to receive(:abort!)
      expect do
        con_checker.error_and_abort!(Jira::Checker::CriticalCheckerError.new(message))
      end.to output("#{' FAIL'.red}\n  #{'- ERROR'.red}: #{message}\n").to_stdout
    end
    ...
    it "should handle Jira::Checker::CheckerCompositeError and return false" do
      ...
      expect do
        output = con_checker.error_and_abort!(Jira::Checker::CheckerCompositeError.new(message,
                                                                                       status: false))
      end.to output("#{' FAIL'.red}\n  #{'- ERROR'.red}: #{message}\n    - status:#{' ✕'.red}\n").to_stdout
      expect(output).to be_falsey
    end
  ...
  it "show_available_issue_types should print 'NO ISSUE TYPES' if no issue types are defined" do
    ...
    expect { con_checker.show_available_issue_types }
      .to output("#{'AVAILABLE ISSUE TYPES: '.yellow}\n  - NO ISSUE TYPES\n").to_stdout
  end
  ...
  it "show_available_issue_states should print list of issue states" do
    ...
    expect { con_checker.show_available_issue_states }
      .to output("#{'AVAILABLE ISSUE STATES:'.yellow}\n  - NAME [id: 1]\n").to_stdout
  end

  it "check_webhook should print warning if MP_HOST env is not set" do
    expect do
      con_checker.check_webhook
    end.to output(
  ...
  it "check_webhook should call checker with MP_HOST env if it's set" do
    ...
    expect do
      con_checker.check_webhook
    end.to output("Checking webhooks for hostname \"#{host}\"...#{' OK'.green}\n").to_stdout
  end
