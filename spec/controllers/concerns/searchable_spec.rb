...
RSpec.describe ApplicationController, type: :controller do
  ...
  let!(:service1) do
    create(:service, name: "duper super name", resource_organisation: providers.first,
                     providers: providers, categories: [categories.first], tag_list: "tag1, tag2")
  end
  let!(:service2) do
    create(:service, name: "very different title", resource_organisation: providers.first,
  ...
  context "#search " do
    ...
    context "with filters and category set" do
      let!(:params) do
        ActionController::Parameters.new("providers" => providers.second.id,
    ...
    context "with filters and category set" do
      let!(:params) do
        ActionController::Parameters.new("providers" => providers.first.id,
                                         "category_id" => categories.second.id)
      end
      it "returns correct service list" do
  ...
  context "#category_counters" do
    ...
    context "with query and filter" do
      let!(:params) do
        ActionController::Parameters.new("q" => "very different",
  ...
  context "filter type tests" do
    ...
    context Filter::Location do
      ...
      let!(:service1) do
        create(:service,
               geographical_availabilities: Array.wrap([Country.load(collection.first),
                                                        Country.load(collection.third)]))
      end
      let!(:service2) do
        create(:service,
               geographical_availabilities: Array.wrap([Country.load(collection.second),
                                                        Country.load(collection.third)]))
      end
