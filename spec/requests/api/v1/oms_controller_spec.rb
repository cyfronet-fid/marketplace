    ...
    get "lists projects" do
      ...
      response 200, "OMS found" do
        ...
        run_test! do |response|
          ...
          expect(data).to eq({ OMS: oms.map { |p| OrderingApi::V1::OmsSerializer.new(p).as_json } }.deep_stringify_keys)
        end
      end

      response 200, "user doesn't administrate any OMS", document: false do
        schema "$ref" => "oms/oms_index.json"
        ...
        run_test! do |response|
          data = JSON.parse(response.body)
          expect(data).to eq({ OMS: [] }.deep_stringify_keys)
        end
      end
