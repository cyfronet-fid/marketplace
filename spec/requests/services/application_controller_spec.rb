...
RSpec.describe "Services::ApplicationController functionality", type: :request do
  ...
  describe "#check_vo_membership!" do
    ...
    context "when the status is not_member" do
      let(:check_vo_membership_result) { Checkin::CheckVoMembership::Result.new(status: :not_member) }

      before do
        allow(Checkin::Config).to receive(:become_vo_member_url).and_return("https://example.com/enroll")
        get service_choose_offer_path(service_resource)
      end
