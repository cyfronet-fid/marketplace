...
RSpec.describe Offer::Update do
  context "#bundled_offers" do
    it "doesn't send notification if offer invalid" do
      ...
      expect { Offer::Update.call(bundle_offer_to_be, { bundled_offers: [bundled_offer] }) }.not_to change {
    ...
    it "doesn't send notification if bundled offers removed" do
      ...
      bundle_offer =
        create(:offer, service: build(:service, resource_organisation: provider2), bundled_offers: [bundled_offer])

      expect { Offer::Update.call(bundle_offer, { bundled_offers: [] }) }.not_to change {
        ActionMailer::Base.deliveries.count
      }
    end
    ...
    it "doesn't send notification if offer non-public" do
      ...
      expect { Offer::Update.call(bundle_offer, { bundled_offers: [bundled_offer] }) }.not_to change {
        ActionMailer::Base.deliveries.count
      }
    end
    ...
    it "sends notification if bundled offers updated" do
      ...
      bundle_offer =
        create(
          ...
          bundled_offers: [bundled_offer1, bundled_offer2]
        )

      expect { Offer::Update.call(bundle_offer, { bundled_offers: [bundled_offer1, bundled_offer3] }) }.to change {
        ActionMailer::Base.deliveries.count
      }.by(1)
    end
