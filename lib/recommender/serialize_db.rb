  ...
  class SerializeDb
    ...
    def call
      { services: Service.all.map { |s| Recommender::ServiceSerializer.new(s).as_json },
        ...
        access_modes: Vocabulary.where(type: "AccessMode").as_json(only: [:id, :name, :description]),
        access_types: Vocabulary.where(type: "AccessType").as_json(only: [:id, :name, :description]),
        trls: Vocabulary.where(type: "Trl").as_json(only: [:id, :name, :description]),
        life_cycle_statuses: Vocabulary.where(type: "LifeCycleStatus").as_json(only: [:id, :name, :description])
      }.as_json
    end
  end
end
