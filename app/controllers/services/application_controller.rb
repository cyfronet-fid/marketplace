...
class Services::ApplicationController < ApplicationController
  ...
  def next_title
    if next_visible_step_key == wizard.step_names.last
      I18n.t("services.order.wizard.next", step_title: step_title(next_visible_step_key)).to_s
    else
      I18n.t("next")
    end
  end
  ...
  def prev_title
    I18n.t("services.order.wizard.previous", step_title: step_title(prev_visible_step_key)).to_s
  end
