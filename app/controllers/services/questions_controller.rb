...
class Services::QuestionsController < ApplicationController
  ...
  def create
    ...
    user = current_user || User.new(first_name: params[:service_question][:author].partition(" ").first,
                                    last_name: params[:service_question][:author].partition(" ").last,
                                    email: params[:service_question][:email])
    @question = Service::Question.new(author: user.full_name,
                                     email: user.email,
