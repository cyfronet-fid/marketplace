...
class UserActionController < ApplicationController
  ...
  def create
    ...
    if cookies[:client_uid].nil?
      cookies.permanent[:client_uid] = SecureRandom.hex(10) + Time.now.getutc.to_i
    end
    ...
    request_body[:source]["visit_id"] += request_body[:unique_id]
    request_body[:target]["visit_id"] += request_body[:unique_id]
