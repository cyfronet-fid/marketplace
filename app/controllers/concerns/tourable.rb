  ...
  private
    def determine_tour
      ...
      if params[:tour].present?
        tour_name = params[:tour] if available_tours.dig(params[:tour], "activation_strategy") == "query_param"
      end
      if tour_name.nil?
        default_tours = available_tours.select do |_, tour|
    ...
    def finished_tours(controller = controller_name, action = action_name)
      ...
      if current_user.present?
        finished |= TourHistory.where(user_id: current_user.id, controller_name: controller, action_name: action)
                               .map(&:tour_name)
      end
