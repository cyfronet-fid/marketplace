...
export default class extends Controller {
  ...
  fill(event){
    ...
    let star_value = event.target.getAttribute("value")
        || this._getParentValue(event.target, "value")
    ...
    for (let el of stars) {
      ...
      star.getAttribute("value") <= value ?
        star.setAttribute('data-prefix','fas') : star.setAttribute('data-prefix','far')
    }
  }

  _getParentValue(element, attribute){
    return element.parentElement.attributes[attribute] ?
      element.parentElement.attributes[attribute].value : null
  }
}
