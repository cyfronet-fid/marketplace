...
class Importers::Logo
  ...
  def call
    Timeout.timeout(10) do
      ...
      filename = @object.pid.presence || to_slug(@object.name)
      ...
      Rails.logger.debug { "ERROR - there was a problem processing image for #{@object.name} #{@image_url}: #{e}" }
    rescue Exception => e
      Rails.logger.debug do
        "ERROR - there was a unexpected problem processing image for #{@object.name} #{@image_url}: #{e}"
      end
    end
  rescue Timeout::Error => e
    Rails.logger.debug { "ERROR - there was a problem with image loading from #{@image_url}: #{e}" }
  end
