...
class Service::Mailer::SendToSubscribers
  ...
  def call
    ...
    subscribers.each do |subscriber|
      ...
      @common_scientific_domains = @service.scientific_domains & subscriber.scientific_domains if subscriber
        .scientific_domains_updates
      if @common_categories.present? || @common_scientific_domains.present?
        ServiceMailer.new_service(@service, @common_categories, @common_scientific_domains, subscriber.email)
          .deliver_later
      end
    end
  end
