...
# Vocabulary types managed in the backoffice. marketplace keeps the set left
# by its V6 migration; pl and whitelabel manage the full set in their order.
# config/initializers/variants.rb and Mp::Variant are not loaded yet here, so
# the variant is read from config/variants.yml directly.
VOCABULARY_TYPES =
  if Rails.application.config_for(:variants)[:current].to_s == "marketplace"
    {
      access_type: {
        name: "Access Type",
