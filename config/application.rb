...
module Mp
  class Application < Rails::Application
    ...
    config.portal_base_url = ENV["PORTAL_BASE_URL"].presence || "https://eosc-portal.eu"

    config.providers_dashboard_url = ENV["PROVIDERS_DASHBOARD_URL"].presence || " https://beta.providers.eosc-portal.eu"

    config.recommender_host = ENV["RECOMMENDER_HOST"]
    config.auth_mock = ENV["AUTH_MOCK"]
    config.eosc_commons_base_url = ENV["EOSC_COMMONS_BASE_URL"].presence || "https://s3.cloud.cyfronet.pl/eosc-portal-common/"
    config.eosc_commons_env = ENV["EOSC_COMMONS_ENV"].presence || "production"
  end
end
