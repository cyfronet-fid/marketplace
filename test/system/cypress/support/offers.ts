...
Cypress.Commands.add('fillFormProject', (project: IProject) => {
    ...
    if (project.reason) {
        cy.get("#project_reason_for_access")
            .clear()
            .type(project.reason);
    }
    ...
    cy.get(".project_scientific_domains")
        .then($el => {
            $el.find(".choices__item--selectable")
    ...
    if (project.customerTypology) {
        cy.get("#project_customer_typology")
            .select(project.customerTypology);
    }

    if (project.email) {
        cy.get("#project_email")
            .clear()
            .type(project.email);
    }

    if (project.organization) {
        cy.get("#project_organization")
            .clear()
            .type(project.organization);
    }

    if (project.department) {
        cy.get("#project_department")
            .clear()
            .type(project.department);
    }

    if (project.countryOfOrigin) {
        cy.get("#project_country_of_origin")
            .select(project.countryOfOrigin);
    }

    if (project.webpage) {
        cy.get("#project_webpage")
            .clear()
...
Cypress.Commands.add("hasProjectDetails", (project: IProject) => {
    ...
    cy.get(".services-menu")
        .find(".nav-link")
        .contains("project details", {matchCase: false})
        .click();
    ...
    cy.get(".additional-information")
        .invoke("text")
        .then(text => Object.keys(project)
            .forEach(key => Array.isArray(project[key])
