...
export type ticketValuesLabels = 'type' | 'status' | 'priority' | 'resolution'
    | 'epic name' | 'ci-displayName' | 'ci-name' | 'ci-surname' | 'ci-email'
    | 'ci-institution' | 'ci-department' | 'ci-departmentalWebPage' | 'cp-customerTypology'
    | 'so-projectName' | 'cp-collaborationCountry' | 'cp-customerCountry' | 'eosc-hub test'
...
Cypress.Commands.add('jiraGetTicketDetailFrom', (label: ticketValuesLabels) => {
    cy.location('pathname')
        .should('include', '/projects/' + Cypress.env('MP_JIRA_PROJECT') + '/issues/');

    return cy.get(".property-list")
        .contains(".item", label, {matchCase: false})
        .find(".value");
});
Cypress.Commands.add('jiraGetEpicIssues', () => {
    cy.jiraGetTicketDetailFrom('type')
        .invoke('text')
        .should('equal', 'Epic');
...
Cypress.Commands.add('jiraOrderTicketsBy', (orderType: ticketOrderType) => {
    ...
    cy.get("#order-by-options-multi-select")
        .then($el => {
            ...
            cy.wrap($el)
                .contains(".item-label", orderType, {matchCase: false})
                .click();
        });
});
Cypress.Commands.add('jiraGetResources', () => {
    return cy.get("#ghx-issues-in-epic-table")
        .then($el => {
...
Cypress.Commands.add('jiraGetTicketComments', () => {
    cy.location('pathname')
        .should('match', /\/projects\/EOSCSO([A-Z]+)\/issues\/EOSCSO([A-Z]+)-[0-9]+\?.+/);
