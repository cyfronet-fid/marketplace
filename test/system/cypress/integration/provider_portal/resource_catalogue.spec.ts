...
describe("Provider Portal - create provider, resource for other catalogue than eosc", () => {
  ...
  it("create provider, resource for other catalogue than eosc and delete resource and provider", {
    retries: {
      runMode: 0,
    ...
    cy.request('GET',`${providerPortalURL}/dashboard/${catalogue}/${provider.name}/resource-dashboard/${provider.name}.${resource.name}/stats`)
    ...
    cy.request({
      method: 'DELETE',
      url: `${providerPortalURL}/api/catalogue/${catalogue}/resource/${provider.name}.${resource.name}`,
      form: false,
