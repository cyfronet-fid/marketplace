                ...
                cy.getCookie('project')
                    ...
                    .then(project => {
                        ...
                        cy.get('select[name="order_type"]')
                            .select("open_access");
                        cy.location('search')
                            .should("include", "order_type=open_access");
                        cy.get(".service-box")
                            .first()
                            .find("h2")
                            .find("a")
            ...
            .then(user => {
                cy.getCookie('project')
                    ...
                    .then(project => {
                        ...
                        cy.get('select[name="order_type"]')
                            .select("order_required");
                        cy.location('search')
                            .should("include", "order_type=order_required");
                        cy.get(".service-box")
                            .first()
                            .find("h2")
                            .find("a")
                        ...
                        cy.checkCaptcha();
                        cy.get("button.btn-primary")
                            .contains("Send access request", {matchCase: false})
                            .first()
                            .click();
                        cy.getCookie('resources')
                            .then(cookie => {
        ...
        cy.getCookie('user')
            ...
            .then(user => {
                cy.getCookie('project')
                    .then(project => JSON.parse(project.value) as IProject)
                    .then(project => {
                        const epicName = 'Project, '
                            + user.last_name + ' '
                            + user.first_name + ', '
                            + project.name;
                        cy.jiraFindTicketBy('name', epicName)
                            .click();
                        cy.waitUntil(() => Cypress.$('.loading').length === 0);

                        cy.jiraGetTicketDetailFrom('priority')
                            .should('include.text', 'Low');
                        cy.jiraGetTicketDetailFrom('status')
                            .should('include.text', 'Active');
                        cy.jiraGetTicketDetailFrom('resolution')
                            .should('include.text', 'Unresolved');
                        cy.jiraGetTicketDetailFrom('epic name')
                            .should('include.text', project.name);
                        cy.jiraGetTicketDetailFrom('ci-displayName')
                            .should('include.text', user.last_name + " " + user.first_name);
                        cy.jiraGetTicketDetailFrom('ci-name')
                            .should('include.text', user.last_name);
                        cy.jiraGetTicketDetailFrom('ci-surname')
                            .should('include.text', user.first_name);
                        cy.jiraGetTicketDetailFrom('ci-email')
                            .should('include.text', project.email);
                        cy.jiraGetTicketDetailFrom('ci-institution')
                            .should('include.text', project.organization);
                        cy.jiraGetTicketDetailFrom('ci-department')
                            .should('include.text', project.department);
                        cy.jiraGetTicketDetailFrom('ci-departmentalWebPage')
                            .should('include.text', project.webpage);
                        cy.jiraGetTicketDetailFrom('cp-customerTypology')
                            .should('include.text', project.customerTypology.toLowerCase());
                        cy.jiraGetTicketDetailFrom('so-projectName')
                            .should('include.text', project.name);
                        cy.jiraGetTicketDetailFrom('cp-collaborationCountry')
                            .should('include.text', 'N/A');
                        cy.jiraGetTicketDetailFrom('cp-customerCountry')
                            .should('include.text', Utilities.capitalizeAll(project.countryOfOrigin));
                        project.scientificDomains
                            .forEach(scientificDomain => {
                                cy.jiraGetTicketDetailFrom('cp-scientificDiscipline')
                                    .should('include.text', Utilities.capitalizeAll(scientificDomain));
                            });
                        cy.jiraGetTicketDetailFrom('eosc-hub test')
                            ...
                            .then(jiraResources => {
                                cy.getCookie('resources')
                                    .then(cookie => {
                                        ...
                                        const appResources: IJiraResource[] = (JSON.parse(cookie.value) as IResource[])
                                            .map(resource => ({
                                                name: "Services order, "
                                                    + user.last_name
                                                    + " " + user.first_name + ","
                                        ...
                                        appResources
                                            .forEach(resource => {
                                                expect(
                                                    jiraResources
                                                        .some(jiraResource => jiraResource.name = resource.name)
                                                ).to.be.equal(true);
                                            });
                                    });
                            });
            ...
            .then(user => {
                cy.getCookie('project')
                    ...
                    .then(project => {
                        ...
                        cy.get("#message_message")
                            .type(message);
                        cy.get(".btn-primary")
                            .contains("Send message", {matchCase: false})
                            .click();
                        cy.get(".frame")
                            .contains(message)
                            .should('exist');
            ...
            .then(user => {
                cy.getCookie('project')
                    ...
                    .then(project => {
                        ...
                        cy.getCookie('message')
                            .then(message => {
                                cy.get("#issue_actions_container")
                                    .find(".action-body")
