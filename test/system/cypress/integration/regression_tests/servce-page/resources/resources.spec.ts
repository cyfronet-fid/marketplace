...
describe("Resources", () => {
  ...
  it("should pin open access offer to a project only once", () => {
    ...
    cy.contains("a", "Pin to a project")
      .click();
    cy.location("href")
      .should("contain", `/services/${openAccessResource.toLocaleLowerCase()}/summary`);
    ...
    cy.contains("button", "Pin!")
      .click();
    cy.contains("h1", openAccessResource)
      .should("be.visible");
    cy.location("href")
    ...
    cy.contains("button", "Pin!")
      .click();
    cy.contains("div", "Project already pinned with this offer")
      .should("be.visible");
  });
  ...
  it("should order order required internal offer many times", () => {
    ...
    cy.contains("button", "Send access request")
      .click();
    cy.contains("h1", orderRequiredResourceInternal)
      .should("be.visible");
    cy.location("href")
      .should("match", /(\/projects\/.*\/services)/);
    cy.contains(".alert-success", "Offer ordered successfully")
      .should("be.visible");
  ...
  it.only("should add resource with few offers", () => {
    ...
    cy.contains("a", "Access instructions")
      .click();
    cy.location("href")
      .should("contain", `/services/${resourceWithFewOffer.toLocaleLowerCase()}/information`);
    cy.contains("a", "Configuration")
      .click();
    cy.location("href")
      .should("contain", `/services/${resourceWithFewOffer.toLocaleLowerCase()}/configuration`);
    cy.get('#parameter_id1')
      .type('1');
    cy.contains("a", "Final details")
      .click();
    cy.location("href")
      .should("contain", `/services/${resourceWithFewOffer.toLocaleLowerCase()}/summary`);
    cy.get("[data-e2e='add-new-project-btn']")
      .click();
    cy.fillFormProject(project2);
    cy.checkCaptcha(2);
    ...
    cy.contains("button", "Send access request")
      .click();
    cy.contains("h1", resourceWithFewOffer)
      .should("be.visible");
    cy.location("href")
      .should("match", /(\/projects\/.*\/services)/);
    cy.contains(".alert-success", "Offer ordered successfully")
